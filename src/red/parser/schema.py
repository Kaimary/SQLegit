...
class Schema:
    ...
    def parse(self):
        ...
        for i, (idx, col) in enumerate(self._column_names_original):
            ...
            if idx != table_idx:
                self.all_tables.append(
                    Table(self._table_names_original[table_idx], cols, pks, col_types)
                )
                table_idx = idx
                cols = []
                col_types = []
        ...
        for fks in self._foreign_keys:
            col_1, col_2 = (
                self._column_names_original[fks[0]],
                self._column_names_original[fks[1]],
            )
            tab_1, tab_2 = (
                self._table_names_original[col_1[0]],
                self._table_names_original[col_2[0]],
            )
    ...
    def get_table(self, table_name: str) -> Table:
        ...
        for table in self.query_tabs:
            if (
                table_name.upper() in table.alias
                or table_name.lower() == table.tab_name.lower()
            ):
                return table
...
class SchemaGraph:
    ...
    def build_graph(self):
        ...
        for fks in schema["foreign_keys"]:
            ...
            self.graph.add_edge(
                tab_1, tab_2, relationship=f"{tab_1}.{col_1[1]} = {tab_2}.{col_2[1]}"
            )
