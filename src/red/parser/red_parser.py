...
def parse_as_unit(tokens: List[SQLToken], db_schema: Schema):
    ...
    for token in tokens:
        if (
            not token.is_left_parenthesis
            and not token.is_right_parenthesis
            and token.parenthesis_level == level
        ):
            all_sub = False
            break
    ...
    if len(tokens) == 1:
        ...
        tokens[0].is_left_parenthesis
        and tokens[0].next_token_not_comment.value.upper() == "SELECT"
        and all_sub
    ):
    ...
    def parse(self):
        ...
        while True:
            ...
            if token is None or token.position == -1:
                ...
                token.parenthesis_level == self._level
                and token.value.upper() in self._SEG
            ):
                break
            ...
            if token.parenthesis_level > self._level:
                ...
                token.value.upper() == "NOT"
                and token.next_token_not_comment.value.upper()
                in {"BETWEEN", "LIKE", "IN"}
            ):
                tmp_seg.insert(0, token)
    ...
    def get_used_schema(self):
        ...
        for op in self.ops:
            if (
                ...
                or isinstance(op, float)
                or isinstance(op, SQLToken)
            ):
                continue
            elif isinstance(op, Column):
    ...
    def cmp_type(self, op):
        ...
        if isinstance(op, Column):
            ...
            if op.exec_table.is_single_column():
                if (
                    "INTEGER" in op.exec_table.cols[0].col_type
                    or "REAL" in op.exec_table.cols[0].col_type
                ):
                    return "NUMERIC"
                else:
        ...
        for i, token in enumerate(self.sql_tokens):
            ...
            if token.parenthesis_level > self.level:
                if i == 0 or (
                    ...
                    and self._is_con(token.previous_token_not_comment)
                ):
                ...
                if is_sub_joint_predicate:
                    self.predicates.append(
                        JointPredicate(tmp_tokens[1:], self.db_schema)
                    )
                is_sub_joint_predicate = False
                tmp_tokens = []
        ...
        for key, conds in col_cond.items():
            if isinstance(key[0], Column):
                ...
                if "REAL" in key[0].col_type or "INTEGER" in key[0].col_type:
                    ...
                    for cond in conds:
                        if cond[1] == idx:
                            if isinstance(cond[0], Predicate) and (
                                isinstance(cond[0].ops[1], float)
                                or isinstance(cond[0].ops[1], int)
                            ):
                                if (
                                    cond[0].CMP == "<" or cond[0].CMP == "<="
                                ) and float(cond[0].ops[1]) < right:
                                    right = float(cond[0].ops[1])
                                elif (
                                    cond[0].CMP[0] == ">" or cond[0].CMP == ">="
                                ) and float(cond[0].ops[1]) > left:
                                    ...
                                    if equal_str == "" and equal == float("inf"):
                                        ...
                                        res.append(
                                            Report(BugLevel.ERROR, self.sql_str, desc)
                                        )
                            elif isinstance(cond[0], Predicate) and isinstance(
                                cond[0].ops[1], str
                            ):
                                if cond[0].CMP == "=":
                                    if equal_str == "" and equal == float("inf"):
                    ...
                    for cond in conds:
                        if cond[1] == idx:
                            if isinstance(cond[0], Predicate) and isinstance(
                                cond[0].ops[1], str
                            ):
                                if cond[0].CMP == "=":
                                    if equal == "":
                            ...
                            if equal != "" and patterns:
                                for pattern in patterns:
                                    ...
                                    if not repat.match(equal):
                                        desc = (
                                            f"The value can't satisfy the need of LIKE"
                                        )
                                        res.append(
                                            Report(BugLevel.INFO, self.sql_str, desc)
                                        )
...
class Expression:
    ...
    def __init__(
        self, sql_tokens: List[SQLToken], db_schema: Schema, exp_type: str = ""
    ):
        self.sql_tokens = sql_tokens
        self.db_schema = db_schema
    ...
    def _detect_exp_type(self):
        if (
            self.sql_tokens[0].value.upper() == "CASE"
            and self.sql_tokens[-1].value.upper() == "END"
        ):
            case_list = [
                token.parenthesis_level == self.level and token.value.upper() == "CASE"
                for token in self.sql_tokens[1:-1]
            ]
            if not any(case_list):
                self.exp_type = "CASE"
        elif self.sql_tokens[0].value.upper() == "CAST":
            cast_list = [
                token.parenthesis_level == self.level for token in self.sql_tokens[1:-1]
            ]
    ...
    def parse(self):
        ...
        if self.exp_type == "CAL":  # Calculation parse
            ...
            for i, token in enumerate(self.sql_tokens):
                if (
                    token.parenthesis_level == self.level
                    and token.value.upper() in self._CMP
                ):
                    is_predicate = True
                    break
            if is_predicate:
                ...
                for i, token in enumerate(self.sql_tokens):
                    ...
                    if token.parenthesis_level > self.level:
                        ...
                        if exp_type == "FUN":
                            self.ops.append(
                                Expression(tmp_tokens, self.db_schema, exp_type)
                            )
                        elif (
                            len(tmp_tokens) > 2
                            and tmp_tokens[1].value.upper() == "SELECT"
                        ):
                            self.ops.append(
                                Query(
                                    sql_tokens_to_sql(tmp_tokens[1:-1]),
                                    deepcopy(self.db_schema),
                                )
                            )
                        else:
                            self.ops.append(
                                Expression(tmp_tokens[1:-1], self.db_schema, exp_type)
                            )
                        tmp_tokens = []
                        exp_type = "CAL"
                        ...
                        token.value.upper() in self._CAL
                        or token.value.upper() in self._CMP
                    ):
            ...
            for i, token in enumerate(self.sql_tokens):
                if (
                    token.value.upper() == "CASE"
                    and token.parenthesis_level == self.level
                ):
                    current = cond_tokens
                elif (
                    token.value.upper() == "WHEN"
                    and token.parenthesis_level == self.level
                ):
                    if then_tokens:
            ...
            for token in self.sql_tokens[2:-1]:
                if (
                    token.value.upper() == ","
                    and token.parenthesis_level == self.level + 1
                ):
    ...
    def validate(self):
        ...
        for op in self.ops:
            if (
                isinstance(op, SQLToken)
                or isinstance(op, Column)
                or isinstance(op, str)
            ):
                continue
            elif (
                isinstance(op, Query)
                or isinstance(op, Expression)
                or isinstance(op, JointPredicate)
            ):
                res.extend(op.validate())
        ...
        if self.exp_type == "CAL":  # Calculation validation
            ...
            if all([isinstance(op, str)] for op in self.ops) and all(
                [op == "," for op in self.ops[1::2]]
            ):
                ...
                for i, op in enumerate(self.ops):
                    if i == 0 or isinstance(op, SQLToken):
                        ...
                        if (
                            isinstance(self.ops[i - 1], SQLToken)
                            and self.ops[i - 1].value.upper() in self._CMP
                        ):
                            self.data_type = "INTEGER"
                        elif right_type == "REAL" and self.data_type != "TEXT":
                            self.data_type = "REAL"
                        elif (
                            right_type == self.data_type == "INTEGER"
                            and isinstance(self.ops[i - 1], SQLToken)
                            and self.ops[i - 1].value == "/"
                        ):
                            desc = (
                                f"Dividing two integers results in loss of precision, "
                            ...
                            cal = self.ops[i - 1].value.upper()
                            desc = (
                                f"Can not operate the {cal} between {self.data_type}({self.ops[i - 2]})"
            ...
            if fun_name not in self.rules:
                ...
                if len(self.ops[1:]) < allowed_num or len(self.ops[1:]) > len(
                    rule["input"]
                ):
                    ...
                    output_type = "TEXT"
                    for i, (parameter, allowed_type) in enumerate(
                        zip(self.ops[1:], rule["input"][: len(self.ops[1:])])
                    ):
                        ...
                        if parameter_type in allowed_type:
                            ...
                            if "|" in rule["output"] and len(allowed_type) == len(
                                rule["output"].split("|")
                            ):
                    ...
                    if output_type == "TEXT":
                        ...
                        for op in self.ops[1:]:
                            ...
                            if col:
                                ...
                                sql = (
                                    f"SELECT {self.sql_str} FROM {tab_name} "
                                    f"WHERE `{col.col_name}` IS NOT NULL LIMIT 1000;"
                                )
                                header, values = execute_sql(
                                    self.db_schema.db_path, sql
                                )
                                if header != "exception":
    ...
    def _is_function(self, token: SQLToken) -> bool:
        return (
            token.value.upper() in self.rules
            and token.next_token_not_comment.is_left_parenthesis
        )
        ...
        for i, token in enumerate(self.sql_tokens):
            ...
            if sub_query:
                self.sub_queries.append(
                    Query(sql_tokens_to_sql(sub_query), deepcopy(self.db_schema))
                )
                self.db_schema.add_query_table(self.sub_queries[-1])
                sub_query = []
            if token.value.upper() == "ON":
                is_table = False
            elif token.is_keyword and (
                "JOIN" in token.value.upper() or "FROM" == token.value.upper()
            ):
                if join_con_tokens:
                    self.join_conds.append(
                        JointPredicate(join_con_tokens, self.db_schema)
                    )
                    join_con_tokens = []
                is_table = True
            elif not is_table:
                ...
                or token.is_alias_of_table_or_alias_of_subquery
        ...
        for joint_cond in self.join_conds:
            ...
            for cond in joint_cond.predicates:
                if isinstance(cond, JointPredicate):
                    ...
                    if not (
                        isinstance(cond.ops[0], Column)
                        or isinstance(cond.ops[0], Expression)
                    ):
                        desc = f"Operand {cond.ops[0]} is not a column;"
                        res.append(Report(BugLevel.WARNING, self.sql_str, desc))
                    if not (
                        isinstance(cond.ops[1], Column)
                        or isinstance(cond.ops[1], Expression)
                    ):
                    ...
                    if isinstance(op0, Column) and isinstance(op1, Column):
                        if not (op0.is_fk_with(op1) or op0 is op1):
                            ...
                            path = self.db_schema.graph.find_minimum_spanning_tree(
                                tables
                            )
                            if path:
                                desc = (
                                    f"No explicitly defined PK-FK relationship between {op0} and {op1};"
        ...
        for op in self.group_cols:
            ...
            if isinstance(op, Column):
                if (
                    ...
                    and len(self.db_schema.query_tabs) == 1
                ):
        ...
        for op in self.order_cols:
            ...
            if isinstance(op, Column):
                ...
                if flag != "exception" and num[0][0] and self.direction[0] == "ASC":
                    desc = (
                        f"The column in `{op.col_name}` has null value and order direction is ASC, "
            ...
            if isinstance(op, Expression):
                ...
                if flag != "exception" and num[0][0] and self.direction[0] == "ASC":
                    desc = (
                        f"The column in `{op.sql_str}` has null value and order direction is ASC, "
                        f"which will make the value of the top results is NULL, "
...
class SelectClause(Clause):
    ...
    def parse(self):
        ...
        for seg in sql_token_seg:
            ...
            alias = None
            if (
                seg[-1].is_alias_definition
                and not seg[-1].is_right_parenthesis
                and not seg[-1].value.upper() == "END"
                and not is_int(seg[-1].value)
            ):
                alias = seg[-1].value
...
class Query:
    ...
    def parser(self):
        ...
        for token in p.tokens:
            t = token.value.upper()
            if (
                t in self._clause_dict
                and current_clause != t
                and token.parenthesis_level == 0
            ):
                # End Clause
                if current_clause:
        ...
        for current_clause in self._parse_order:
            if current_clause in self.clauses_tokens:
                ...
                self.clauses[current_clause] = clause_type(
                    self.clauses_tokens[current_clause], self.db_schema, **self.clauses
                )
                pass
    ...
    def exec_table(self) -> Table:
        ...
        self.table = Table(
            f"TMP{str(time.time())[8:]}".replace(".", ""), col_names, [], col_types
        )
    ...
    def validate(self):
        ...
        if is_null_table:
            if "(select max(" in self.sql.lower():
                ...
                desc = (
                    f"The execution result of SQL is null. "
            ...
            if len(self.exec_table.cols) == 1:
                if (
                    len(set(self.exec_table.cols[0].values)) == 1
                    and len(self.exec_table.cols[0].values) > 1
                ):
                    desc = (
                        f"There is only single column and single value in the execution results."
                ...
                if "SELECT" in self.clauses:
                    if isinstance(self.clauses["SELECT"].projections[0], Expression):
                        if self.clauses["SELECT"].projections[0].exp_type == "FUN":
                            if "COUNT" in self.clauses["SELECT"].projections[
                                0
                            ].sql_str and (
                                float(self.exec_table.cols[0].values[0]) == 0
                            ):
                                desc = (
                                    f"The execution result of COUNT() of SQL is zero. "
                                    f"\n[REFINE SUGGESTION]: No tuple satisfy all the predicate, wrong column might be included."
                                )
                                res.append(Report(BugLevel.INFO, self.sql, desc))
